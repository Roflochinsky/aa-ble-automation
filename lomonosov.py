# 🚀 ТАЙМЛАЙНЫ AA_BLE (окно 6:00–21:00, ПОМИНУТНО)
...
# - Справа отдельная «карточка» со временем по ЗОНАМ за окно 6–21
print("🔄 Запуск анализа aa_ble (6:00–21:00, поминутно, панель времени по зонам)…")
...
def create_user_timeline_chart_window(df_user, tag_desc_map, row_height=60, window_start=(6,0), window_end=(21,0)):
    ...
    for (tag, z), g in df.groupby(['ble_tag','zone_id']):
        ...
        for _, r in g.iterrows():
            hovers.append("<br>".join([
                ...
                title,
                f"Зона: {r['zone_name']}",
                f"Всего в этой метке (6–21): {tot} мин",
    ...
    fig.update_layout(
        ...
        plot_bgcolor='white',
        paper_bgcolor='rgba(0,0,0,0)',  # fix: вместо 'transparent'
        margin=dict(l=110, r=20, t=70, b=80),
        barmode='overlay', hoverlabel=dict(font=dict(size=10))
    )
    fig.update_xaxes(
        title=dict(text="Время (6:00–21:00)", font=dict(size=16, color="#333")),
    ...
    # Панель справа: время по зонам (8–22)
    ...
    for zid in order:
        ...
        name = {0:"Вне зоны BLE-маячков",1:"Зоны проведения работ",2:"Столовые",3:"Опасные зоны",4:"Курилки",5:"Зоны отдыха",6:"ВЖГ",7:"Туалеты",8:"Остановки автобусов",9:"Административные помещения",10:"Зона выдачи WW",11:"Склад",12:"Мастерские",13:"КПП"}.get(zid, f"Зона {zid}")
        color = PALETTES.get(zid, PALETTES['other'])[0]
        items.append(f"<div class='zone-row'><span class='dot' style='background:{color}'></span><span class='zname'>{name}</span><span class='zval'>{mins} мин</span></div>")
    panel_html = f"""
    <div class="zone-card">
      <div class="zone-title">Время по зонам (6:00–21:00)</div>
...
<style>
 ...
 .zone-row {{ display:flex; align-items:center; justify-content:space-between; padding:6px 4px; border-bottom:1px dashed #eee; }}
 .zone-row:last-child {{ border-bottom:none; }}
 .dot {{ display:inline-block; width:12px; height:12px; border-radius:50%; margin-right:8px; }}
 .zname {{ flex:1; margin-left:8px; }}
 .zval {{ margin-left:12px; font-weight:bold; color:#333; }}
...
def export_overall_excel_period(overall_by_date: dict):
    ...
    filename = f"AA_BLE Таймлайны 6-21 {min_d.replace('.','-')}—{max_d.replace('.','-')}.xlsx"
    ...
    with pd.ExcelWriter(filename, engine='openpyxl') as w:
        for the_date in dates_sorted:
            ...
            sh_e = f"{tag} — Таблица 6-21"; sh_z = f"{tag} — Зоны 6-21"
...
def create_aable_presence_analysis():
    print("=" * 80)
    print("🏢 ТАЙМЛАЙНЫ AA_BLE (6:00–21:00, поминутно; справа — окно «Время по зонам»)")
    print("=" * 80)
    try:
    ...
    for the_date in all_dates:
        ...
        we = datetime.combine(display_date, dtime(21,0))
    ...
    if all_dates:
        if len(all_dates)==1:
            page_title = f"AA_BLE Таймлайны (6–21) — {date_str}"
            filename = f"AA_BLE Таймлайны 6-21 {date_str}.html"
        else:
            ...
            page_title = f"AA_BLE Таймлайны (6–21) — период {min_d}–{max_d}"
            filename = f"AA_BLE Таймлайны 6-21 {min_d}—{max_d}.html"
        export_combined_html(combined_sections, filename, page_title)

    export_overall_excel_period(overall_by_date)
    print("\n✅ ГОТОВО! Окно 6:00–21:00, поминутный учёт; справа — отдельное окно со временем по ЗОНАМ; на диаграмме метки, окрашенные в оттенки своей зоны.")
    print("Повторная выгрузка отдельных графиков: redownload_last_charts().")

print("=" * 100)
print("🏢 AA_BLE ТАЙМЛАЙНЫ (6:00–21:00, поминутно; справа — окно «Время по зонам»)")
print("=" * 100)
